...
const DIFFICULTIES = [
	{ id: "easy", label: "EASY", cpuSpeed: 0.012, errorRate: 0.22 },
	{ id: "normal", label: "NORMAL", cpuSpeed: 0.015, errorRate: 0.12 },
	{ id: "hard", label: "HARD", cpuSpeed: 0.018, errorRate: 0.06 }
];

const BALL_SPEEDS = [
	{ id: "slow", label: "SLOW", multiplier: 0.8 },
	{ id: "normal", label: "NORMAL", multiplier: 1.0 },
	{ id: "fast", label: "FAST", multiplier: 1.2 }
];

const COURT_THEMES = [
	{ id: "classic", label: "CLASSIC", courtA: "#c47a3d", courtB: "#b06a30" },
	{ id: "night", label: "NIGHT", courtA: "#4a5568", courtB: "#3a4556" },
	{ id: "forest", label: "FOREST", courtA: "#6b4423", courtB: "#5a3a1a" },
	{ id: "royal", label: "ROYAL", courtA: "#8b5a8e", courtB: "#6a3a6a" }
];

const SPLASH_DIFF_BUTTON = {
	x: 18,
	y: 220,
	w: 56,
	h: 24,
...
const SPLASH_SETTINGS_BTN = { x: 30, y: 300, w: 160, h: 24 };
...
class AIPlayer extends Player {
	...
	setDifficulty(level) {
		this.speed = level.cpuSpeed;
		this.errorRate = level.errorRate;
	}
	trackBall(ball) {
...
class InputManager {
	constructor(canvas) {
		...
		this.cb = { swing: null, any: null, uiHit: null, pause: null };
		this.touch = null;
...
class Scoreboard {
	...
	_winGame(idx) {
		...
		this.matchOver = this.games[idx] >= this.gamesToWin;
		this.winner = idx;
	}
	pointDisplay() {
...
class Game {
	constructor(canvas) {
		...
		this.cpu.setDifficulty(this.difficulty);

		// New options and statistics
		this.ballSpeedIndex = 1;
		this.ballSpeedMultiplier = BALL_SPEEDS[this.ballSpeedIndex].multiplier;
		this.courtThemeIndex = 0;
		this.courtTheme = COURT_THEMES[this.courtThemeIndex];
		this.rallyCount = 0;
		this.maxRallyCount = 0;
		this.playerStrokeCount = 0;
		this.cpuStrokeCount = 0;
	...
	_difficultyButtonBounds(idx) {
		return {
			x: SPLASH_DIFF_BUTTON.x + idx * (SPLASH_DIFF_BUTTON.w + SPLASH_DIFF_BUTTON.gap),
			y: SPLASH_DIFF_BUTTON.y,
			w: SPLASH_DIFF_BUTTON.w,
			h: SPLASH_DIFF_BUTTON.h
		};
	}

	_difficultyButtonAt(cx, cy) {
		for (let i = 0; i < DIFFICULTIES.length; i++) {
			const b = this._difficultyButtonBounds(i);
	...
	_ballSpeedButtonAt(cx, cy) {
		const x = 20, y = 140, w = 50, h = 22, gap = 8;
		for (let i = 0; i < BALL_SPEEDS.length; i++) {
			const bx = x + i * (w + gap);
			if (cx >= bx && cx <= bx + w && cy >= y && cy <= y + h) return i;
		}
		return -1;
	}

	_courtThemeButtonAt(cx, cy) {
		const x = 20, y = 200, w = 50, h = 22, gap = 8;
		for (let i = 0; i < COURT_THEMES.length; i++) {
			const bx = x + i * (w + gap);
			if (cx >= bx && cx <= bx + w && cy >= y && cy <= y + h) return i;
		}
		return -1;
	...
	_uiHit(cx, cy) {
		...
		if (this.state === STATE.SETTINGS) {
			...
			const ballSpeedBtn = this._ballSpeedButtonAt(cx, cy);
			if (ballSpeedBtn !== -1) {
				this.ballSpeedIndex = ballSpeedBtn;
				this.ballSpeedMultiplier = BALL_SPEEDS[ballSpeedBtn].multiplier;
				return true;
			}
			const courtThemeBtn = this._courtThemeButtonAt(cx, cy);
			if (courtThemeBtn !== -1) {
				this.courtThemeIndex = courtThemeBtn;
				this.courtTheme = COURT_THEMES[courtThemeBtn];
				return true;
			}
			return true;
		}
		...
		if (this.state === STATE.SPLASH) {
			const diffBtn = this._difficultyButtonAt(cx, cy);
			if (diffBtn !== -1) {
				this._setDifficulty(diffBtn);
				return true;
			}
			...
			if (this._inBtn(cx, cy, SPLASH_SETTINGS_BTN)) {
	...
	_togglePause() {
		if (this.state === STATE.PAUSED) {
			this.state = this.prevState || STATE.RALLYING;
			this.prevState = null;
			return;
		}
		if (
	...
	_drawDifficultyButtons(ctx) {
		for (let i = 0; i < DIFFICULTIES.length; i++) {
			const b = this._difficultyButtonBounds(i);
			ctx.fillStyle = this.difficultyIndex === i ? COLORS.accent : "rgba(0,0,0,0.55)";
			ctx.fillRect(b.x, b.y, b.w, b.h);
			ctx.fillStyle = COLORS.accent;
			ctx.fillRect(b.x, b.y, b.w, 1);
			ctx.fillRect(b.x, b.y + b.h - 1, b.w, 1);
			...
			ctx.fillStyle = this.difficultyIndex === i ? "#000" : "#fff";
			ctx.fillText(DIFFICULTIES[i].label, b.x + b.w / 2, b.y + 16);
		}
		ctx.font = '8px "Press Start 2P", monospace';
		ctx.textAlign = "center";
		ctx.fillStyle = "#fff";
		ctx.fillText("DIFFICULTY", W / 2, SPLASH_DIFF_BUTTON.y - 8);
	}
	...
	_drawSplashSettingsButton(ctx) {
		const b = SPLASH_SETTINGS_BTN;
		ctx.fillStyle = "rgba(0,0,0,0.55)";
		ctx.fillRect(b.x, b.y, b.w, b.h);
		ctx.fillStyle = COLORS.accent;
		ctx.fillRect(b.x, b.y, b.w, 1);
		...
		ctx.textAlign = "center";
		ctx.fillStyle = "#fff";
		ctx.fillText("OPTIONS", b.x + b.w / 2, b.y + 16);
	}
	...
	onSwing(dir) {
		...
		this.ball.vx *= this.ballSpeedMultiplier;
		this.ball.vy *= this.ballSpeedMultiplier;
		this.playerStrokeCount++;
		this.rallyCount++;
		this.sfx.hit();
	}
	...
	_startMatch() {
		this.score.reset();
		this._setDifficulty(this.difficultyIndex);
		this.maxRallyCount = 0;
		this.playerStrokeCount = 0;
		this.cpuStrokeCount = 0;
		this._startPoint();
	...
	draw() {
		...
		if (this.state === STATE.PAUSED) this._drawPauseOverlay();
	}
	...
	_drawHUD() {
		...
		ctx.fillStyle = COLORS.ball;
		const matchLabel = this.score.gamesToWin > 1 ? `BEST OF ${this.score.gamesToWin * 2 - 1}` : "1 GAME MATCH";
		ctx.fillText(matchLabel, W / 2, 22);

		ctx.fillStyle = COLORS.ball;
		if (this.score.serverIdx === 0) ctx.fillRect(34, 8, 3, 3);
		else ctx.fillRect(W - 37, 8, 3, 3);

		// Draw rally counter and ball speed
		ctx.font = '6px "Press Start 2P", monospace';
		ctx.textAlign = "left";
		ctx.fillStyle = COLORS.ball;
		ctx.fillText("RALLY:" + this.rallyCount, 6, 22);
		
		const speedLabel = BALL_SPEEDS[this.ballSpeedIndex].label[0];
		ctx.textAlign = "right";
		ctx.fillText("SPEED:" + speedLabel, W - 6, 22);
	}
	...
	_drawPauseOverlay() {
		...
		ctx.font = '8px "Press Start 2P", monospace';
		ctx.fillText("PRESS P TO RESUME", W / 2, H / 2 + 12);
	}
	...
	_drawSplash() {
		...
		this._drawDifficultyButtons(ctx);
		this._drawSplashSoundButton(ctx);
		this._drawSplashSettingsButton(ctx);
		this._drawSplashStartButton(ctx);
	}
	...
	_drawSettings() {
		...
		ctx.fillStyle = "#fff";
		ctx.textAlign = "left";
		ctx.fillText("BALL SPEED", 20, 125);
		this._drawBallSpeedButtons(ctx);

		// Court Theme options
		ctx.fillText("COURT THEME", 20, 185);
		this._drawCourtThemeButtons(ctx);

		// Back button
		ctx.fillStyle = COLORS.accent;
		ctx.fillRect(SPLASH_START_BTN.x, SPLASH_START_BTN.y - 40, SPLASH_START_BTN.w, SPLASH_START_BTN.h);
		ctx.fillStyle = "#000";
		ctx.font = '12px "Press Start 2P", monospace';
		ctx.textAlign = "center";
		ctx.fillText("BACK", W / 2, SPLASH_START_BTN.y - 20);
	}

	_drawBallSpeedButtons(ctx) {
		const x = 20, y = 140, w = 50, h = 22, gap = 8;
		for (let i = 0; i < BALL_SPEEDS.length; i++) {
			const bx = x + i * (w + gap);
			ctx.fillStyle = this.ballSpeedIndex === i ? COLORS.accent : "rgba(0,0,0,0.55)";
			ctx.fillRect(bx, y, w, h);
			ctx.fillStyle = COLORS.accent;
			ctx.fillRect(bx, y, w, 1);
			ctx.fillRect(bx, y + h - 1, w, 1);
			ctx.fillRect(bx, y, 1, h);
			ctx.fillRect(bx + w - 1, y, 1, h);
			ctx.font = '6px "Press Start 2P", monospace';
			ctx.textAlign = "center";
			ctx.fillStyle = this.ballSpeedIndex === i ? "#000" : "#fff";
			ctx.fillText(BALL_SPEEDS[i].label, bx + w / 2, y + 14);
		}
	}

	_drawCourtThemeButtons(ctx) {
		const x = 20, y = 200, w = 50, h = 22, gap = 8;
		for (let i = 0; i < COURT_THEMES.length; i++) {
			const bx = x + i * (w + gap);
			ctx.fillStyle = this.courtThemeIndex === i ? COLORS.accent : "rgba(0,0,0,0.55)";
			ctx.fillRect(bx, y, w, h);
			ctx.fillStyle = COLORS.accent;
			ctx.fillRect(bx, y, w, 1);
			ctx.fillRect(bx, y + h - 1, w, 1);
			...
			ctx.font = '6px "Press Start 2P", monospace';
			ctx.textAlign = "center";
			ctx.fillStyle = this.courtThemeIndex === i ? "#000" : "#fff";
			ctx.fillText(COURT_THEMES[i].label, bx + w / 2, y + 14);
		}
	}
}
